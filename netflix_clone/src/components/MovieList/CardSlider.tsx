      ...
      >
        <div className='flex items-center cursor-pointer group/title'>
          ...
          <div className='flex items-center text-[#54b9c5] text-[0.9vw] font-medium opacity-0 group-hover:opacity-100 transition-opacity ml-2'>
            <span
              className={`transition-all duration-500 overflow-hidden whitespace-nowrap translate-y-[5px] ${isTitleHovered ? 'max-w-[100px] opacity-100' : 'max-w-0 opacity-0'}`}
            >
              모두보기
            </span>
          ...
          {extendedMovies.map((movie, index) =>
            variant === 'ranking' ? (
              <RankingCard
                ...
                movie={movie}
                rank={
                  ((index - startOffset + movies.length) % movies.length) + 1
                }
